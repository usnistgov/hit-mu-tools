angular.module('edi').factory('EDICursorUtils',
    ['$rootScope', '$http', '$q', 'EDIEditorUtils', function ($rootScope, $http, $q, EDIEditorUtils) {
        return  {
            /**
             *
             * @param active
            ...
            getCoordinate: function (editor) {
                try {
                    if (editor != null) {
                        var separators = EDIEditorUtils.getSeparators();
...
angular.module('edi').factory('EDIEditorUtils',
    ['$rootScope', '$http', '$q', function ($rootScope, $http, $q) {
        return  {
            ...
            getSeparators: function () {
                return [ '|', '^', '&', '~' ]; // get this from the message parser
            },
...
angular.module('edi').factory('EDIUtils', function () {
...
angular.module('edi').factory('EDITreeUtils',
    ['$rootScope', '$http', '$q', 'EDICursorUtils', 'EDIUtils', function ($rootScope, $http, $q, EDICursorUtils, EDIUtils) {
        return  {
...
//                        return this.findLastChild(tree, node);
            ...
            getStringValue: function (type, path, segment) {
                     var position = EDIUtils.getPosition(path, type);
                    var instanceNumber = EDIUtils.getInstanceNumber(path, type);
                    switch (type) {
                        case "SEGMENT":
                        {
