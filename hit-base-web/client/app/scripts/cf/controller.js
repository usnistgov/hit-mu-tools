...
angular.module('cf')
    .controller('CFValidatorCtrl', ['$scope', '$http', 'CF', '$window', 'HL7EditorUtils', 'HL7CursorUtils', '$timeout', 'HL7TreeUtils', '$modal', 'NewValidationResult', 'HL7Utils', '$rootScope', 'MessageValidator', 'MessageParser', 'ValidationResultHighlighter', function ($scope, $http, CF, $window, HL7EditorUtils, HL7CursorUtils, $timeout, HL7TreeUtils, $modal, NewValidationResult, HL7Utils, $rootScope, MessageValidator, MessageParser, ValidationResultHighlighter) {
...
//
        ...
        $scope.parseMessage = function () {
            ...
            if ($scope.cf.testCase.testContext.profile != null && $scope.cf.message.content != '') {
                var parsed = new MessageParser().parse($scope.cf.testCase.testContext.id, $scope.cf.message.content);
...
angular.module('cf')
    .controller('CFVocabularyTabCtrl', ['$scope', 'CF', '$timeout', function ($scope, CF, $timeout) {
        ...
        $scope.init = function (valueSetGroup) {
            if (valueSetGroup) {
                $scope.tableLibrary = valueSetGroup;
                $scope.tableList = valueSetGroup.children;
                $scope.tmpList = [].concat($scope.tableList);
            }
        };
    }]);
